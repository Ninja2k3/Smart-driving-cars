    ...
    try:
        ...
        if len(boxes) > 1:
            ...
            if w1 > 110 and w2 > 110:
                ...
                line_length_curr = int(((x1 + (w1 / 2) - x2 - (w2 / 2)) ** 2 + (y1 + (h1 / 2) - y2 -
                                        (h2 / 2)) ** 2) ** 0.5)
                if line_length_curr < line_length_prev / 2:
                    continue
                cv2.line(img, (x1 + int(w1 / 2), y1 + int(h1 / 2)), (x2 + int(w2 / 2), y2 + int(h2 / 2)), (255, 0, 0),
                         3)
                min = (h2, h1)[h1 < h2]
                max = (h1, h2)[h1 < h2]
                cv2.putText(img, "current distance between them : " + str(line_length_curr), (10, 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                cv2.putText(img, 'minor object height : ' + str(min), (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (0, 0, 255), 3)
                cv2.putText(img, 'major object height : ' + str(max), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (0, 0, 255), 3)
